...
// TARGETING HIGHLIGHT
// ═══════════════════════════════════════════════════════════
const computeHighlight = (step, board, r, c) => {
  const sz = board?.length || 0;
  if (step==="cell" && board) return getFlood(board, r, c);
...
const reducer = (s, a) => {
  switch (a.type) {
    ...
    case "CLICK_CELL": {
      ...
      if (s.targeting) {
        const {step}=s.targeting;
        if(step==="cell") return {...s,targeting:{...s.targeting,step:"color_final",cell:[r,c]},highlight:computeHighlight("cell",s.board,r,c),message:"Now pick a color"};
        if(step==="row")  return {...s,targeting:{...s.targeting,step:"color_final",row:r},highlight:computeHighlight("row",s.board,r,c),message:"Now pick a color"};
        if(step==="col")  return {...s,targeting:{...s.targeting,step:"color_final",col:c},highlight:computeHighlight("col",s.board,r,c),message:"Now pick a color"};
        return s;
      }
    ...
    const onUp = (e) => {
      ...
      if (curDrag && curHover) {
        ...
        if (card && !card.special && ["cell+color","row+color","col+color"].includes(card.target)) {
          ...
          setTimeout(() => dispatch({ type: "CLICK_CELL", r: hr, c: hc }), 10);
        }
      }
            ...
            return (
              ...
              >
                <CardView card={card} playable={playable}
                  onClick={playable ? ()=>{ if(didDrag.current) { didDrag.current=false; return; } dispatch({type:"SELECT_CARD",idx:i}); } : undefined} />
              </div>
            );
